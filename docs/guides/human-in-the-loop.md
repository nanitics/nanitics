...
> **Coverage:** tool-call granularity ships for `ReActAgent`; orchestration-step granularity (a whole agent as one step) holds for **every** agent type through `Sequential`, and `Loop` carries the same per-iteration cursor. Finer granularity for the other agent types (ReWOO, Reflexion, ToT, LATS, CodeAct) and the per-step cursor for the remaining orchestrators (`Conditional`, `Parallel`, `DAG`) are in progress — until then those resume at the coarser boundary with a wider in-flight window, never incorrectly.
