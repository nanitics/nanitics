...
Every execution unit in a workflow implements the `Step` protocol (a `name` property and an async `execute` method). The SDK provides three adapters: `AgentStep` wraps an agent, `FunctionStep` wraps an async function, and `WorkflowStep` wraps a nested workflow. All return `StepResult`, which carries an `output` (passed between steps), a `metadata` dict (step counts, termination reasons, intermediate results), and a typed `usage: Usage | None` field (token usage produced by the step — `None` for `FunctionStep` and other non-LLM steps). When the wrapped agent has `output_schema`, `AgentStep` forwards the parsed Pydantic model as the step output; the text response is available in `metadata["text_output"]`. See the source docstrings for details and [examples/workflows/sequential_pipeline.py](../../examples/workflows/sequential_pipeline.py) Section 2b for structured output flow.

> **Per-step and per-workflow-run usage.** Read token usage off `StepResult.usage` (typed `Usage | None`). `Sequential` and `Pipeline` attach an aggregated `usage` to their returned `StepResult` — the sum of every sub-step's `usage`. The legacy `metadata["usage"]` dict mirror written by `AgentStep` and the bound agent/handoff step wrappers is deprecated and will be removed in 1.0.0. See [docs/migrations/step-result-usage.md](../migrations/step-result-usage.md).
...
**Crash resume (opt-in).** The flow above writes a checkpoint only when a step suspends for a human. To also resume a run that *crashed* without suspending, set `step_checkpoints=True`: a thin cursor plus a journal record are written after each completed step (and, for a `ReActAgent`, each completed tool batch), and the run resumes with no human response via `resume_interrupted` / `resume_from_checkpoint`. Covered today by `Sequential`, `Loop`, and the concurrent orchestrators `Parallel` and `DAG` — per branch / per node. Because branches and nodes complete in arbitrary order, the concurrent orchestrators reconstruct the completed set on resume from the journal (an order-independent union keyed by step path) rather than from the cursor, so concurrent completions never clobber each other; the replay window generalizes to all branches/nodes in flight at crash time. The guarantee is at-least-once with a one-step replay window per in-flight step, so side-effecting tools must be idempotent. See [Step-level durability (crash resume)](human-in-the-loop.md#step-level-durability-crash-resume).

**Limitation:** In `Parallel`, `DAG`, and `MapReduce`, only the first concurrent *HITL suspension* is captured. This is unchanged by crash resume — the journal-backed `step_checkpoints` path is independent and records every completed branch/node regardless of order. If multiple branches might need human *approval*, sequence them instead.
