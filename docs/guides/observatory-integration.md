...
and panel registration, and closes with what the v0.1.0 Observatory is
...
It is **not** a production observability platform. At v0.1.0 there is
...
not on npm at v0.1.0 (see
[What is not shipped at v0.1.0](#what-is-not-shipped-at-v010)).
...
The Observatory at v0.1.0 is a dev tool. Running it in production is
...
v0.1.0. Protect the mount path (`/api/observatory`) and the UI path
(`/api/observatory/`) together. A future
`ObservatoryAuthProvider` protocol is post-launch and signal-driven
(see [What is not shipped](#what-is-not-shipped-at-v010)).
...
not ship a multi-tenant store — the Observatory at v0.1.0 assumes a
single scope.
