...
> **Namespace:** `ReActAgent`, `ReasoningAgent`, and `CodeActAgent` live in top-level `nanitics`. The specialized strategies — `ReWOOAgent`, `ReflexionAgent`, `TreeOfThoughtAgent`, `LATSAgent` — live in `nanitics.specialized`. Import accordingly.
