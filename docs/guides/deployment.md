...
- A serverless chapter. Long-running agent loops, durable HITL, and streaming traces have awkward shapes on request-per-function runtimes; the SDK does not ship a serverless adapter at v0.1.0.
...
The compose exposes `app` on port 8000 with no authentication in front of it. The Observatory at v0.1.0 has no built-in auth hook on `create_observatory_router` — that seam is deliberate, and filling it is adopter-owned at v0.1.0.
...
> For the full Observatory production posture — the four seams (auth, multi-tenancy, retention, content scrubbing), who owns each, and why no default `ObservatoryAuthProvider` ships at v0.1.0 — see [Observatory Integration](observatory-integration.md#for-production).
