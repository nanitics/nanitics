...
- Service-dependent tests (marked `docker`) are skipped in CI for v0.1.0. Run them locally with `just check docker=true` (or `just ci`) when Docker is available. Real-service tests live in `validation/` and are run via `just validate`.
